...
- **CORS, un error real encontrado al probarlo en el celular del usuario**: la primera versión de la Edge Function no contestaba la consulta previa de permiso (`OPTIONS`) que hace el navegador antes de llamar a otro dominio, ni mandaba las cabeceras `Access-Control-Allow-*`. Resultado: el botón "Probar aviso del servidor" fallaba con "Failed to send a request to the Edge Function" —un error de red genérico, que no dice nada— sin que la llamada llegara nunca al servidor. La tarea programada no tenía este problema porque es servidor a servidor. Corregido: se contesta el `OPTIONS` y **todas** las respuestas llevan las cabeceras (una sola función `responder()`, para que no vuelva a olvidarse en una rama).
- **El servidor de aplicación se arma cuando hace falta, no al arrancar** (`obtenerAppServer()`): si el secreto `VAPID_JWK` estuviera mal pegado, la función devuelve un mensaje que se entiende en vez de no levantar (un fallo de arranque en una Edge Function se ve desde la app como otro error de red indistinguible).
- Probado: `push.test.ts` (conversión de claves, que si falla rompe el cifrado sin dar error claro), `swNotificaciones.test.ts` (ejecuta de verdad `public/sw-notificaciones.js` con un `self` simulado y le dispara eventos `push` y `notificationclick`) y `edgeFunction.test.ts`, que **ejecuta la Edge Function** con un `Deno` y unas dependencias de mentira (se le cambian los imports de JSR por dobles de prueba y se la transforma con esbuild) para mandarle consultas como las del navegador y las del cron. Es la única forma de probar automáticamente, sin Deno instalado, lo que corre del lado del servidor; se verificó que la prueba de CORS efectivamente falla si se le quita el manejo del `OPTIONS`.
...
- [~] **Notificaciones push desde el servidor**: código de la app, SQL y Edge Function listos y probados (160/160 pruebas automáticas, verificado en la build de producción). El usuario ya corrió el SQL, guardó los secretos y creó la función; la primera prueba en su Android falló por el CORS que faltaba (ver arriba). **Pendiente**: que vuelva a pegar la función corregida y confirme con el botón "Probar aviso del servidor".
